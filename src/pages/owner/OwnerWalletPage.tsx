...
type TxType = 'deposit' | 'withdraw' | 'prize' | 'other';

const TX_CONFIG: Record<TxType, { color: string; bg: string; label: string; icon: React.ElementType }> = {
  deposit:  { color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20', label: 'Deposit',         icon: ArrowDownLeft },
  ...
  other:    { color: 'text-purple-400',  bg: 'bg-purple-500/10 border-purple-500/20',   label: 'Other',           icon: Coins },
};
...
export function OwnerWalletPage() {
  ...
  async function handleDeposit() {
    ...
    try {
      ...
      setDepositMsg(`Deposit of $${effectiveUsd} USD (${coinsPreview.toLocaleString()} coins) successful!`);
  ...
  async function handleWithdraw() {
    ...
    try {
      ...
      setWithdrawMsg(`Withdrawal of $${amt} USD (${coinsToWithdraw.toLocaleString()} coins) successful!`);
      setWithdrawInput('');
      loadAll();
    } catch (err: unknown) {
  ...
  const prizeTotal = transactions
    .filter(t => normalizeType(t.type ?? t.transactionType ?? '') === 'prize')
    ...
    <div className="min-h-screen text-body font-sans flex" style={{ backgroundColor: '#0b101e' }}>
      ...
      <div className="relative flex-1 min-w-0 overflow-y-auto">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          ...
          <PageHero
            title="My Wallet"
            subtitle="Manage balance, deposit/withdraw funds, and track tournament prizes"
          ...
          <motion.div variants={stagger} initial="hidden" animate="show" className="grid grid-cols-3 gap-4">
              ...
              <div className="relative z-10">
                ...
                {pageLoading ? <LoadingSkeleton /> : (
                  <>
                    ...
                    <div className="text-sm text-muted">${(balance / COINS_PER_USD).toFixed(2)} USD equivalent</div>
                  </>
                )}
              </div>
            </motion.div>
            ...
            <motion.div variants={child} className="glass-panel rounded-xl p-5 relative overflow-hidden border border-amber-500/20">
              ...
              <div className="relative z-10 text-xl font-serif font-bold text-amber-400">
                {pageLoading ? '…' : `+${prizeTotal.toLocaleString()}`}
              </div>
              <div className="relative z-10 text-[11px] text-muted font-medium mt-1">Total Prize Money</div>
            </motion.div>
            ...
            <motion.div variants={child} className="glass-panel rounded-xl p-5 relative overflow-hidden">
              ...
              <div className="w-9 h-9 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 mb-3 relative z-10">
                <CheckCircle size={16} />
              </div>
              <div className="relative z-10 text-xl font-serif font-bold text-emerald-400">
          ...
          <div className="grid grid-cols-[400px_1fr] gap-6">
            ...
            <div className="space-y-4">
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-panel rounded-xl p-6 relative overflow-hidden">
                ...
                <div className="mb-4">
                  <div className="text-[11px] text-muted font-medium mb-2 uppercase tracking-wider">Quick Select</div>
                ...
                <button onClick={handleOpenVNPay} disabled={coinsPreview <= 0 || depositLoading || isLocked}
                  ...
                  {isLocked ? '🔒 Deposits Disabled' : depositLoading ? 'Processing...' : coinsPreview > 0 ? `Deposit $${effectiveUsd} → ${coinsPreview.toLocaleString()} coins` : 'Select Amount'}
                </button>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="glass-panel rounded-xl p-6 relative overflow-hidden">
                ...
                <button onClick={handleWithdraw} disabled={withdrawLoading || !withdrawInput || parseFloat(withdrawInput) <= 0}
                  ...
                  {withdrawLoading ? 'Processing...' : parseFloat(withdrawInput) > 0 ? `Withdraw $${withdrawInput} → ${(parseFloat(withdrawInput) * COINS_PER_USD).toLocaleString()} coins` : 'Withdraw'}
                </button>
              </motion.div>
            </div>
                      ...
                      <motion.div
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {isPrize && (
                              <span className="shrink-0 text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-amber-500/15 text-amber-400 border border-amber-500/30 uppercase tracking-wider">
                                Prize
                              </span>
                            )}
                          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-sm bg-[#0B101E] border border-gold-border/40 rounded-2xl shadow-2xl overflow-hidden relative flex flex-col animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-glass-border flex items-center justify-between bg-white/[0.01]">
              ...
              <button onClick={() => setShowVNPayModal(false)} className="text-muted hover:text-white text-xs font-semibold uppercase cursor-pointer">
                Close
              </button>
            </div>
            <div className="p-6 space-y-5 overflow-y-auto max-h-[75vh]">
              ...
              <div className="space-y-3 bg-white/[0.02] border border-glass-border rounded-xl p-4 text-xs">
                ...
                <div className="flex justify-between py-1 border-t border-glass-border/40">
                  ...
                  <span className="text-white font-semibold">{`EQUESTRIA OWNER ${user?.userId ?? 0}`}</span>
                </div>
              </div>
            ...
            <div className="px-6 py-4 border-t border-glass-border flex items-center justify-end gap-3 bg-white/[0.01]">
              <button onClick={() => setShowVNPayModal(false)} className="px-4 py-2 border border-glass-border hover:bg-white/[0.04] text-muted hover:text-white rounded-lg text-xs font-bold transition-all cursor-pointer">
                Cancel
              </button>
              <button onClick={handleDeposit} className="px-4 py-2 bg-gold hover:bg-gold-hover text-black rounded-lg text-xs font-extrabold shadow-lg shadow-gold/20 hover:shadow-gold/30 transition-all cursor-pointer">
