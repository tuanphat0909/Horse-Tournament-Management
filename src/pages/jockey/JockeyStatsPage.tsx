...
export function JockeyStatsPage() {
  ...
  const statsDisplay = [
    { label: 'Total Wins', value: loading ? '...' : wins, icon: Trophy, color: 'text-gold', bg: 'from-gold/15 to-amber-900/20' },
    { label: 'Total races', value: loading ? '...' : totalRaces, icon: Flag, color: 'text-blue-400', bg: 'from-blue-500/15 to-blue-900/20' },
    { label: 'Win Rate', value: loading ? '...' : `${winRate}%`, icon: TrendingUp, color: 'text-emerald-400', bg: 'from-emerald-500/15 to-emerald-900/20' },
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr_320px] gap-6">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-panel rounded-xl p-6 relative overflow-hidden">
              ...
              <div className="relative z-10 flex items-center gap-3 mb-5">
                ...
                <h2 className="text-base font-serif text-white">Result Distribution</h2>
              ...
              {loading ? (
                ...
                <div className="glass-panel rounded-xl p-12 text-center relative overflow-hidden">
                  ...
                  <div className="text-muted text-sm">No races participated yet</div>
                </div>
              ) : (
                <div className="space-y-4 py-2">
                  <div>
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-white">1st Place finishes</span>
                  ...
                  <div>
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-white">Other positions</span>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-panel rounded-xl p-6 flex flex-col items-center justify-center text-center relative overflow-hidden">
              ...
              <div className="text-4xl font-serif font-bold text-white mb-1">
                {loading ? '—' : rankingPoint > 0 ? `#${Math.max(1, 20 - Math.floor(rankingPoint / 10))}` : 'Unranked'}
              </div>
              <div className="text-sm text-muted mb-4">Provisional personal rank</div>
              <div className="w-full p-3 rounded-xl bg-gold/5 border border-gold/20">
                <div className="text-xs text-muted mb-1">Accumulated points</div>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="glass-panel rounded-xl overflow-hidden relative">
            ...
            <div className="p-5 border-b border-glass-border relative z-10 flex items-center gap-3">
              ...
              <h2 className="text-base font-serif text-white">History races tham gia</h2>
              <div className="flex-1 h-px bg-gradient-to-r from-gold/30 via-glass-border to-transparent" />
            </div>
            <div className="p-6">
              {loading ? (
                ...
                <div className="glass-panel rounded-xl p-12 text-center relative overflow-hidden">
                  <div className="absolute top-0 left-6 right-6 h-px bg-gradient-to-r from-transparent via-gold/40 to-transparent pointer-events-none" />
                  <div className="text-4xl opacity-40 mb-3">📊</div>
                  <div className="text-muted text-sm">No race history</div>
                </div>
              ) : (
                <div className="text-sm text-white space-y-4">
                  <div className="p-4 bg-white/[0.02] border border-glass-border rounded-lg flex items-center justify-between">
                    <div>
                      <div className="font-semibold text-gold">Participated in {totalRaces} official races</div>
                      <div className="text-xs text-muted">Win rate: {winRate}%</div>
                    </div>
                    <div className="font-mono text-sm text-gold">+{wins * 10} bonus points</div>
                  </div>

                  {/* Danh sách các races đã completed */}
