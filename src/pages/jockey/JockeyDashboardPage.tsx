          ...
          <motion.div variants={stagger} initial="hidden" animate="show" className="grid grid-cols-4 gap-4">
            {[
              ...
              <motion.div key={i} variants={child} onClick={() => navigate(m.path)}
                ...
                <div className={`absolute -top-4 -right-4 w-24 h-24 rounded-full bg-linear-to-br ${m.bg} blur-[30px] opacity-60 group-hover:opacity-100 transition-opacity pointer-events-none`} />
          ...
          <div className="grid grid-cols-[1fr_380px] gap-6">
              ...
              {loading ? (
                ...
                <div className="space-y-3">
                  {pending.slice(0, 5).map((c, i) => (
                    <div key={c.id ?? i} className="flex items-center gap-4 p-4 rounded-xl bg-white/2 border border-yellow-500/15 hover:border-yellow-500/30 hover:bg-gold/4 transition-all group">
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-panel rounded-xl p-6 relative overflow-hidden">
              ...
              <div className="relative z-10 flex items-center gap-3 mb-5">
                ...
                <h2 className="text-lg font-serif text-white">Thống kê mùa giải</h2>
                <div className="flex-1 h-px bg-linear-to-r from-gold/30 via-glass-border to-transparent" />
              </div>
              {Object.keys(stats).length === 0 ? (
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { label: 'Tổng cuộc đua', value: stats.totalRaces ?? stats.races ?? '—' },
                    { label: 'Số lần thắng', value: stats.wins ?? '—' },
                    { label: 'Số lần thua', value: stats.losses ?? '—' },
                    { label: 'Tỷ lệ thắng', value: stats.winRate != null ? `${stats.winRate}%` : '—' },
                    { label: 'Điểm xếp hạng', value: stats.rankingPoint ?? stats.points ?? '—' },
                    { label: 'Năm kinh nghiệm', value: stats.experienceYears != null ? `${stats.experienceYears} năm` : '—' },
                  ].map((item, i) => (
                    <div key={i} className="p-3 rounded-xl bg-white/2 border border-glass-border">
                      <div className="text-[10px] text-muted font-medium uppercase tracking-wider mb-1">{item.label}</div>
                      <div className="text-base font-bold text-champagne">{String(item.value)}</div>
                    </div>
                  ))}
                </div>
              )}
