    ...
    <div className="min-h-screen text-body font-sans flex" style={{backgroundColor: '#0b101e'}}>
      ...
      <div className="flex-1 relative min-w-0 overflow-y-auto">
        ...
        <main className="relative z-10 max-w-[1600px] mx-auto px-8 py-6 space-y-6">
          ...
          {loading ? (
            <div className="text-center py-12 text-muted">{t('Loading statistics...')}</div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                >
                  <div className="space-y-2">
                    <span className="text-xs font-bold text-muted uppercase">{t('Awaiting Inspection')}</span>
                ...
                <div className="glass-panel rounded-xl p-6 flex items-center justify-between">
                  <div className="space-y-2">
                    <span className="text-xs font-bold text-muted uppercase">{t('Performed')}</span>
                ...
                <div className="glass-panel rounded-xl p-6 flex items-center justify-between">
                  <div className="space-y-2">
                    <span className="text-xs font-bold text-muted uppercase">{t('Medical Standard Met')}</span>
                ...
                <div className="glass-panel rounded-xl p-6 flex items-center justify-between">
                  <div className="space-y-2">
                    <span className="text-xs font-bold text-muted uppercase">{t('Suspected Doping')}</span>
              ...
              <div className="glass-panel rounded-xl p-8 relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="space-y-1">
                  <h4 className="font-serif text-lg font-bold text-champagne">{t('Start Horse Health Inspection')}</h4>
                  <p className="text-sm text-muted">{t('View the list of horses approved for registration but not yet clinically examined.')}</p>
                </div>
                <button
                  onClick={() => navigate('/vet/medical-check')}
                ...
                >
                  {t('Start Inspection')} <ChevronRight size={16} />
                </button>
              </div>
