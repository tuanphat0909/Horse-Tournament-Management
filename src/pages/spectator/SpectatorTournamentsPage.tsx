...
export function SpectatorTournamentsPage() {
  ...
  async function openDetail(t: any) {
    ...
    try {
      const d: any = await getTournamentDetail(id);
      setDetail(d?.result ?? d ?? null);
    } catch (err: unknown) {
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {detail.rounds.map((r: any, ri: number) => (
                            <div key={r.roundId ?? ri} className="flex items-center justify-between text-xs px-3 py-2 rounded-lg bg-white/[0.03] border border-glass-border">
